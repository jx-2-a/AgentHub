 ...
 * 按等级区分:info 左对齐 / important 重要消息(带框+底色+markdown) / choice 选择卡(左对齐) / welcome 欢迎卡(居中) / hint 提示 / prompt 系统询问(彩色)。
 * info/important/hint 是自由文本日志(常带表格/列表的中间结论)→ markdown 渲染;welcome 是 ASCII art、choice 是等宽菜单、
 * prompt 是短询问,保持原样。 */
export function SystemBubble({ text, level, sid }: { text: string; level: LogLevel; sid?: string | null }) {
  const rich = level === 'info' || level === 'important' || level === 'hint';
  return (
